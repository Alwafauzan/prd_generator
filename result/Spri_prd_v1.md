...
- **RJ**: SPRI diterbitkan dokter spesialis/DPJP saat pasien dinilai butuh observasi, tindakan, atau terapi lanjutan yang tidak bisa rawat jalan. Dokter melengkapi diagnosis, indikasi, DPJP, kelas perawatan, dan kebutuhan khusus.
- **IGD**: SPRI diterbitkan dokter jaga/spesialis untuk pasien gawat darurat yang setelah stabilisasi dinyatakan butuh rawat inap. Mencakup diagnosis kerja, indikasi medis, kebutuhan isolasi, tingkat ketergantungan, dan preferensi kelas.
...
3. Form SPRI berisi: data pasien (autofill), diagnosis & indikasi, DPJP ranap, kelas perawatan (diinginkan/dijamin), kebutuhan isolasi, tingkat ketergantungan, kebutuhan khusus.
...
Modul lain (luar cluster): Admisi Rawat Inap (TPPRI), Master Bed/Ruang, Pendaftaran & Penjamin (SEP).
...
5. Dokter isi data klinis: diagnosis, indikasi rawat inap, DPJP ranap, kelas (diinginkan & dijamin), kebutuhan isolasi, tingkat ketergantungan, kebutuhan khusus.
...
10. **Event akhir (modul ini)**: *SPRI berhasil diterbitkan & terkirim ke admisi*. Proses penempatan kamar & transfer (E11) di luar scope.
11. Bila keputusan berubah (mis. ternyata rujuk eksternal) → dokter **batalkan SPRI** dengan alasan; status `Dibatalkan`.
12. Modul Discharge (E13) membaca flag `SPRI ada?` otomatis.
...
[Start: Pasien selesai diperiksa di poli]
  ...
  → Dokter isi: diagnosis, indikasi, DPJP, kelas (diinginkan/dijamin), kebutuhan khusus
...
[Start: Pasien gawat darurat di IGD]
  ...
  → Sistem autofill identitas pasien
  → Dokter isi: diagnosis kerja/sementara, indikasi, kebutuhan isolasi,
...
- **BR-03**: SPRI **tidak dapat diterbitkan** bila data wajib (diagnosis, indikasi, DPJP ranap, kelas dijamin) belum lengkap. (gateway *Data Lengkap & Valid?*)
...
- **US-004** — Sebagai **Dokter**, saya ingin melihat ketersediaan & kelas tempat tidur saat membuat SPRI, agar memilih kelas yang realistis. [ASUMSI]
...
- **US-006** — Sebagai **Petugas Admisi/TPPRI**, saya ingin menerima SPRI elektronik beserta data klinis & preferensi kelas, agar penempatan kamar cepat & sesuai. (source: handoff antarunit)
...
| **FR-003** | Sistem menyediakan form input data klinis SPRI (diagnosis, indikasi, DPJP ranap, kelas, isolasi, tingkat ketergantungan, kebutuhan khusus). | US-001/002, layar di Data Req §11.A |
...
| **FR-009** | Sistem menyediakan daftar SPRI per unit dengan filter status/tanggal/pasien (dashboard, Data Req §11.B). | US-008 |
...
| **FR-013** | Sistem mencetak SPRI ke PDF dengan kop RS, data pasien, klinis, DPJP, tanda tangan dokter. | US-010 |
...
| kode_diagnosis | Kode Diagnosis (ICD-10) | lookup | Tidak | format ICD-10 | master ICD-10 | [ASUMSI] mapping SATUSEHAT |
| indikasi_ranap | Indikasi Rawat Inap | textarea | Ya | maks 500 char | manual | alasan medis |
| kelas_diinginkan | Kelas Diinginkan | dropdown | Tidak | enum kelas (VIP/1/2/3) | manual | BR-05 |
| kelas_dijamin | Kelas Dijamin | dropdown | Ya | enum kelas, sesuai hak penjamin | integrasi BPJS / aturan penjamin | BR-05/06 |
| naik_kelas | Naik Kelas? | boolean | Tidak | true bila diinginkan > dijamin | auto-hitung | FR-006 [PERLU KONFIRMASI] |
| kebutuhan_isolasi | Kebutuhan Isolasi | dropdown | Tidak | enum: Tidak/Airborne/Droplet/Kontak | manual | terutama IGD |
| tingkat_ketergantungan | Tingkat Ketergantungan | dropdown | Tidak | enum: Minimal/Partial/Total [PERLU KONFIRMASI skala] | manual | terutama IGD |
| kebutuhan_khusus | Kebutuhan Pelayanan Khusus | textarea | Tidak | maks 255 char | manual | mis. ICU/HCU, oksigen |
| ruang_diminta | Ruang/Bangsal Diminta | dropdown(lookup) | Tidak | master ruang ranap | lookup | informasi utk admisi |
...
### 11.B Panel TAMPIL — Ketersediaan Tempat Tidur (FR-004, read-only) [ASUMSI sumber master bed]

| Kolom | Sumber Data | Format Tampilan | Filter/Sort | Catatan |
|-------|-------------|-----------------|-------------|---------|
| Ruang/Bangsal | master_ruang | text | filter | |
| Kelas | master_ruang.kelas | badge | filter | VIP/1/2/3 |
| Bed Tersedia | agregasi bed kosong | angka (hijau bila >0) | sort | |
| Bed Terisi | agregasi bed terisi | angka | – | |
| Kapasitas | master_ruang.kapasitas | angka | – | |
| Isolasi | master_ruang.flag_isolasi | badge | filter | cocokkan kebutuhan_isolasi |

### 11.C Layar TAMPIL — Daftar/Dashboard SPRI per Unit (FR-009)

| Kolom | Sumber Data | Format Tampilan | Filter/Sort | Catatan |
|-------|-------------|-----------------|-------------|---------|
| No. SPRI | spri.no_spri | text | sort | |
| No. RM | spri.no_rm | text | filter | |
| Nama Pasien | spri.nama | text | sort A-Z | |
| Unit Asal | spri.unit_asal | badge (RJ/IGD) | filter | |
| DPJP Ranap | spri.dpjp_ranap | text | filter | |
| Kelas Dijamin | spri.kelas_dijamin | badge | filter | |
| Status | spri.status | badge (Diterbitkan/Menunggu Bed/Diproses Admisi/Dibatalkan) | filter | warna per status |
| Tgl Terbit | spri.tgl_terbit | dd-mm-yyyy HH:mm | sort desc (default) | |
| Aksi | – | tombol (Lihat/Edit/Batal/Cetak) | – | Edit/Batal sesuai BR-07 |

Kartu ringkasan: **Total SPRI Hari Ini**, **Menunggu Bed**, **Diproses Admisi** (count agregasi).
...
- **NFR-006 (Interoperabilitas)**: Diagnosis pakai ICD-10; siap dipetakan ke SATUSEHAT (Encounter/Condition) saat fase integrasi. [ASUMSI]
...
| **Master Bed / Ruang (internal)** | Konsumsi | Tampil ketersediaan & kelas bed | Wajib (FR-004) [ASUMSI sumber data] |
...
| **SATUSEHAT** | Kirim | Kirim Encounter/Condition rawat inap (diagnosis ICD-10) | Fase 2 [ASUMSI] |
...
- Pemetaan diagnosis ke SATUSEHAT (Encounter/Condition) adalah fase integrasi lanjutan.
...
- Skala 'tingkat ketergantungan' yang dipakai RS (mis. minimal/partial/total care atau skala lain)?
...
- Apakah SPRI perlu tanda tangan elektronik dokter (e-sign) untuk keabsahan dokumen?