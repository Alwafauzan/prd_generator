...
**Versi:** 1.3 - Pengembangan: konfirmasi usulan — singkatan kode kategori final (UMUM/BPJS/ASPEM/ASWAS), Kategori read-only setelah dibuat (DIKONFIRMASI), keunikan Nama bersifat global lintas-kategori (DIKONFIRMASI), Alamat/Email/No. Telepon bersifat opsional (DIKONFIRMASI). Mewarisi v1.2 (kategori fixed, prefix tetap 'PENJAMIN', penomoran kode per-kategori PENJAMIN-<KAT>-XXXX.
...
| 4 | Phase 1 | Aktif/Nonaktifkan Tipe Penjamin |
...
5. Meningkatkan efisiensi & akurasi pengelolaan data penjamin antar modul.
...
| 6 | Keunikan Nama (global) | 0% duplikasi Nama tipe penjamin lintas seluruh kategori (BR-001) |
...
- `g-admisi-inpatient-registration` — gateway **"Pasien BPJS?"** → percabangan billing bergantung pada tipe penjamin.
...
| **BR-012** | Saat impor (Phase 2): baris dengan Nama duplikat **global** di-skip/ditolak sesuai Mode (tambah vs tambah+update); kolom Kode pada file diabaikan (kode di-generate sistem per kategori); baris dengan **kategori di luar daftar fixed ditolak**; Alamat/No. Telepon/Email boleh kosong (opsional). | Lampiran — Phase 2 |
| **BR-013** | **Kategori bersifat read-only setelah data dibuat** (tidak dapat diubah saat Edit) agar kode `PENJAMIN-<KAT>-<XXXX>` tetap konsisten dengan kategorinya. Tidak ada fitur pindah kategori; bila perlu, buat data baru & nonaktifkan data lama. | **Dikonfirmasi v1.3** (Kategori tetap read-only) |
| **BR-014** | Pemetaan **kode singkat kategori** (`<KATEGORI>` pada kode) mengikuti tabel baku **final**: Umum/Pribadi→`UMUM`, BPJS→`BPJS`, Asuransi Pemerintah→`ASPEM`, Asuransi Swasta→`ASWAS`. | **Dikonfirmasi v1.3** (sesuai usulan) |
...
| **US-011** | Sebagai **Admin Master Data**, saya ingin dapat **mengisi data secara cepat dengan hanya field wajib** (Nama, Kategori, Status) dan **mengosongkan Alamat/No. Telepon/Email** bila belum tersedia, agar input tidak terhambat. | P1 | BR-005 |
...
| **FR-007** | Sistem menyediakan **toggle Status Aktif/Nonaktif**. Untuk **Kategori BPJS** | P0 | Nonaktif → tidak muncul di pendaftaran/billing; data historis utuh. | US-004/011; BR-003/004/012 |
...
| **FR-009** | Sistem mencatat **audit trail** untuk tambah/ubah/nonaktif (user, timestamp, before/after) | P1 | Log dapat ditelusuri. | US-010/011; BR-006/012 |
...
| status_aktif | Status | boolean | **Ya** | aktif / nonaktif | default **aktif** | field kanonik; BR-003|
...
### A. Layar INPUT — Impor Data (Phase 2, FR-012)
...
### B. Layar TAMPIL — Dashboard Tipe Penjamin (FR-001)
...
### C. Layar TAMPIL — Detail Tipe Penjamin
...
> Tiap field di atas tertaut ke FR-005/006/013 (form & generate kode per kategori), FR-001 (dashboard), FR-009 (audit). Field yang tidak pasti ditandai `[PERLU KONFIRMASI]` — tidak dikosongkan.
...
> Tidak ada panggilan API eksternal (BPJS/SATUSEHAT/Disdukcapil) **langsung** dari modul ini.
